...
async function enter(role: Role) {
  error.value = ''
  try { await auth.enterAs(role, role === 'faculty' ? facultyAccessCode.value.trim() : undefined); await router.push(role === 'student' ? '/student' : '/faculty') }
  catch (e) { error.value = apiError(e, 'The local API is not available. Start the server and try again.') }
}
</script>
  ...
  <main id="main-content" class="landing">
    ...
    <section class="landing__hero">
      ...
      <div class="role-panel card">
        <div class="role-panel__head"><span>{{ locale.t('Choose your workspace') }}</span><small>{{ locale.t('Student preview needs no sign-in; hosted faculty access may require a code') }}</small></div>
        <button class="role-choice" :disabled="auth.loading" @click="enter('student')"><span class="role-choice__icon"><GraduationCap /></span><span><b>{{ locale.t('Enter as Student') }}</b><small>{{ locale.t('Practise cases and review your feedback') }}</small></span><ArrowRight class="role-choice__arrow" /></button>
        <label class="faculty-access"><span>{{ locale.t('Faculty access code') }}</span><input v-model="facultyAccessCode" class="input" type="password" autocomplete="current-password" :placeholder="locale.t('Required on the hosted preview')"></label>
        <button class="role-choice" :disabled="auth.loading" @click="enter('faculty')"><span class="role-choice__icon role-choice__icon--faculty"><Stethoscope /></span><span><b>{{ locale.t('Enter as Faculty') }}</b><small>{{ locale.t('Manage cases, rubrics and results') }}</small></span><ArrowRight class="role-choice__arrow" /></button>
        <div v-if="auth.loading" class="landing__loading"><span class="spinner"></span>{{ locale.t('Preparing your workspace…') }}</div>
        <div v-if="error" class="alert alert--error error-box">{{ error }}</div>
