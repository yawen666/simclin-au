...
const error = ref('')
const notice = ref(route.query.evaluation === 'started'
  ? locale.t('Your consultation has ended. Feedback is being generated in the background; you can leave this page and return later.')
  : '')
...
function schedulePoll() {
  ...
  if (sessions.value.some((session) => session.status === 'evaluating')) {
    ...
    notice.value = sessions.value.some((session) => session.status === 'evaluation_failed')
      ? locale.t('One feedback report could not be generated. You can retry it below.')
      : locale.t('Your formative feedback is ready. Select the completed attempt to review it.')
  }
}
...
async function retryEvaluation(session: ClinicalSession) {
  ...
  try {
    ...
    notice.value = locale.t('Feedback generation restarted. You can leave this page and return later.')
    schedulePoll()
  } catch (cause) {
    ...
    <section v-else class="history-list">
      <article v-for="item in filtered" :key="item.id" class="card history-row">
        ...
        <div class="history-copy">
          ...
          <span><CalendarDays :size="13"/>{{ new Date(item.startedAt).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' }) }} · {{ Math.round((item.durationSeconds || 0) / 60) }} {{ locale.t('min') }}</span>
          <small>{{ stateMessage(item) }}</small>
        </div>
        <StatusPill :value="item.status"/>
        <ScoreRing v-if="item.score != null" :score="item.score" size="small"/>
        <RouterLink v-if="item.status === 'completed' && item.resultId" class="history-action" :to="`/student/feedback/${item.resultId}`" :aria-label="locale.t('View feedback')">
          <ChevronRight :size="18"/>
        </RouterLink>
        <RouterLink v-else-if="item.status === 'active'" class="history-action" :to="`/student/consultation/${item.id}`" :aria-label="locale.t('Continue consultation')">
          <ChevronRight :size="18"/>
        </RouterLink>
...
<style scoped>
.history-notice{display:flex;align-items:center;gap:9px;margin-bottom:14px}.filter-row{justify-content:space-between}.history-list{display:grid;gap:10px}.history-row{display:grid;grid-template-columns:auto minmax(0,1fr) auto auto auto;gap:17px;align-items:center;padding:15px 18px}.history-icon{width:42px;height:42px;display:grid;place-items:center;border-radius:11px;background:var(--green-soft);color:var(--green)}.history-copy{min-width:0}.history-copy h3{margin:0}.history-copy>span{display:flex;align-items:center;gap:5px;color:var(--muted);font-size:10px}.history-copy small{display:block;margin-top:5px;color:#66756f;font-size:10px;white-space:normal}.history-action{width:34px;height:34px;display:grid;place-items:center;border-radius:8px;color:var(--green)}.history-action:hover{background:var(--green-soft)}.history-progress{width:34px;height:34px;display:grid;place-items:center}.history-progress .spinner{width:18px;height:18px}.history-retry{white-space:nowrap}
@media(max-width:700px){.history-row{grid-template-columns:auto minmax(0,1fr) auto}.history-row .score-ring{display:none}.history-row .status-pill{grid-column:2}.history-row>.history-action,.history-row>.history-progress,.history-row>.history-retry{grid-row:1/3;grid-column:3}.history-copy small{padding-right:4px}}
</style>
