  ...
  <div class="page"><RouterLink class="text-button" to="/student/history"><ArrowLeft :size="16"/> {{ locale.t('Practice history') }}</RouterLink>
    ...
    <template v-else-if="result">
      ...
      <section class="card scoring-method">
        ...
        <div v-if="result.capApplied != null" class="scoring-cap"><ShieldCheck :size="17"/><span><b>{{ locale.t('Safety cap applied') }}: {{ result.capApplied }}/100.</b> {{ locale.t(result.scoreCapReason || 'A safety-critical history element was not elicited.') }}</span></div>
