...
import { ArrowLeft, Check, MessageSquareQuote, Save } from '@lucide/vue'
  ...
  <div class="page"><RouterLink class="text-button" to="/faculty/results"><ArrowLeft :size="16"/> {{ locale.t('Student results') }}</RouterLink>
    ...
    <template v-else-if="result">
      ...
      <div class="review-layout"><main>
        <section class="card card--padded"><div class="section-title"><div><h2>{{ locale.t('Assessment evidence') }}</h2><span class="subtitle">{{ locale.t('Select a quote to locate the original turn in the transcript.') }}</span></div></div><article v-for="criterion in result.criteria" :key="criterion.criterionId" class="evidence"><div class="evidence-head"><span><b>{{ criterion.name }}</b><small :class="['evidence-status', `evidence-status--${criterion.evidenceStatus ?? 'not_asked'}`]">{{ evidenceStatusLabel(criterion.evidenceStatus) }}</small></span><strong>{{ criterion.score }} / {{ criterion.maxScore }}</strong></div><MarkdownContent :content="criterion.feedback"/><p v-if="!criterion.evidence.length" class="evidence-empty">{{ evidenceStatusLabel(criterion.evidenceStatus) }}</p><button v-for="e in criterion.evidence" :key="e.turnId" type="button" class="evidence-quote" @click="focusTurn(e.turnId)"><MessageSquareQuote :size="14"/> “{{ e.quote }}” <cite>{{ locale.t('Turn') }} {{ e.turnId }}</cite></button></article></section>
        <section class="card card--padded transcript"><h2>{{ locale.t('Consultation transcript') }}</h2><div v-for="turn in result.transcript" :id="`review-turn-${turn.id}`" :key="turn.id" :class="['transcript-turn', { 'transcript-turn--highlight': highlightedTurn === turn.id }]" ><div><b>{{ locale.t(turn.role === 'student' ? 'Student' : 'Patient') }}</b><small>{{ locale.t('Turn') }} {{ turn.id }}</small></div><p>{{ turn.content }}</p></div></section>
...
<style scoped>
.result-head{display:flex;justify-content:space-between;align-items:center;gap:25px;margin:20px 0 30px}.result-head__score{display:flex;align-items:center;gap:15px}.result-head__score .score-ring{width:105px}.score-comparison{display:flex;align-items:center;gap:8px;font-size:10px;color:var(--muted)}.score-comparison span{display:grid}.score-comparison b{font-size:18px;color:var(--ink)}.score-comparison i{font-style:normal;color:var(--green)}.review-layout{display:grid;grid-template-columns:minmax(0,1fr) 320px;gap:18px;margin-top:18px}.review-layout main{display:grid;gap:18px}.review-layout aside{position:sticky;top:25px;height:fit-content}.evidence{padding:16px 0;border-bottom:1px solid var(--line)}.evidence-head{display:flex;justify-content:space-between;align-items:center}.evidence-head>span{display:flex;align-items:center;gap:8px}.evidence-head strong{color:var(--green)}.evidence-status{font-size:9px;padding:2px 7px;border-radius:20px;font-weight:500;background:#eef2f0}.evidence-status--covered{color:#2e765b;background:#e7f3ed}.evidence-status--asked_no_credit{color:#996521;background:#fff3df}.evidence-status--not_asked{color:#7b8580;background:#eef1f0}.evidence-empty{font-size:12px;color:var(--muted);margin:10px 0}.evidence-quote{display:block;width:100%;margin:8px 0 0;padding:10px;border:0;border-left:3px solid var(--sage);background:#f5f8f6;text-align:left;font-size:11px;cursor:pointer;color:#596761}.evidence-quote:hover{background:#eaf3ee}.evidence-quote svg{vertical-align:middle;margin-right:5px}.evidence-quote cite{color:#8a9691}.transcript>div{padding:12px 0;border-bottom:1px solid var(--line)}.transcript-turn>div{display:flex;justify-content:space-between}.transcript-turn b{font-size:10px;text-transform:uppercase;color:var(--green)}.transcript-turn small{font-size:9px;color:var(--muted)}.transcript-turn p{margin:4px 0;font-size:13px}.transcript-turn--highlight{background:#fff7dc;box-shadow:0 0 0 2px #e6bd52;padding:12px;border-radius:8px}.override{display:grid;gap:14px}.override h2{margin:0}.override>p{color:var(--muted);font-size:11px}@media(max-width:850px){.review-layout{grid-template-columns:1fr}.review-layout aside{position:static;grid-row:1}.result-head{align-items:flex-start;flex-direction:column}.result-head__score{flex-wrap:wrap}.result-head__score .score-ring{width:85px}}
</style>
