...
打开 `http://localhost:5173`，API 健康检查为 `http://localhost:4100/api/health`。学生无需密码，每个浏览器使用具有稳定合成姓名、相互隔离且可重置的匿名身份；当前线上演示的教师入口同样为一键进入：
...
- [ ] `/api/health` 返回 `status=ok`、`database=ok`、`runtime=python`、`schemaVersion=5`、`facultyAccessMode=open-demo` 和 `facultyAccessProtected=false`，不返回任何密钥或访问码内容。
- [ ] 前端不包含教师访问码；当前 open-demo 教师工作区按公开演示管理。关闭 open-demo 后，生产环境缺少 `FACULTY_DEMO_ACCESS_CODE` 时必须启动失败。
