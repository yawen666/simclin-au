...
| FastAPI/SQLite `pytest` 单元与 API 契约 | 通过 | 71/71；包含旧 Node schema 迁移、代理/容量保护、完整 open-demo 教师权限、合成学生姓名迁移和 online-E2E validator |
...
- 当前公开演示通过服务端 `open-demo` 模式提供完整教师权限，前端不携带访问码；关闭该模式后生产环境必须配置教师访问码。DeepSeek key 始终只保存在服务端。
