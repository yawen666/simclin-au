...
export function historyRoutes(db: AppDatabase): FastifyPluginAsync {
  return async (app) => {
    ...
    app.get('/', async (request) => {
      ...
      const rows = db.prepare(`SELECT s.id,s.case_id AS caseId,c.title,c.specialty,s.status,s.started_at AS startedAt,
        s.completed_at AS completedAt,s.duration_seconds AS durationSeconds,
      ...
      return { history: rows.map((row) => ({ ...row, result: row.status === 'completed' ? serializeResult(db, Number(row.id)) : null })) };
    });
  };
}
